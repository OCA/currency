...
class ResCurrency(models.Model):
    ...
    monthly_rate = fields.Float(
        compute="_compute_current_monthly_rate",
        string="Current Monthly Rate",
        digits=(12, 6),
        help="The monthly rate of the currency to " "the currency of rate 1.",
    )
    monthly_rate_ids = fields.One2many(
    ...
    def _select_currencies(self, date=None):
        ...
        query = """SELECT c.id, (SELECT r.rate
                                 ...
                                 OR r.company_id = %s::boolean::integer)
    ...
    def _get_conversion_rate(self, from_currency, to_currency, company, date):
        ...
        return to_currency._get_current_monthly_rate(
            date
...
class ResCurrencyRateMonthly(models.Model):
    ...
    name = fields.Date(compute="_compute_name", store=True, required=True, index=True)
    year = fields.Char(
        size=4, required=True, default=lambda self: self._default_get_year()
    )
    ...
    def _compute_name(self):
        for rate in self:
            rate.name = fields.Date.from_string("%s-%s-01" % (rate.year, rate.month))
    ...
    _sql_constraints = [
        ...
        (
            "unique_year_month",
            "unique (year,month,currency_id,company_id)",
            "Only one currency monthly rate per month allowed!",
        ),
    ]
