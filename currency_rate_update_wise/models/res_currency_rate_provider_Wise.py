...
class ResCurrencyRateProviderWise(models.Model):
    ...
    def _wise_check_error(self, data):
        ...
        if isinstance(data, dict) and data.get("error"):
            raise UserError(data.get("error_description") or _("Unknown error"))
    ...
    def _wise_provider_retrieve(self, url, params=None):
        ...
        try:
            ...
            raise UserError(_("Could not reach Wise.com: %s") % e) from e
        except ValueError as e:  # invalid JSON
            raise UserError(_("Unexpected (non-JSON) response from Wise.com")) from e
