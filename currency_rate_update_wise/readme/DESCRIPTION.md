This module adds [Wise.com](https://wise.com/) as a currency exchange rates
...
- **Rate quality**: Wise returns the interbank mid-market rate. On a working
  day, aligned with the ECB fixing, it tracks the ECB reference very closely
  (closer than xe.com in our measurements) and applies no markup to the
  published rate.
...
- **Stable API**: rates are fetched through an authenticated JSON API rather
  than by scraping a web page, which keeps updates robust over time.
- **Efficient and historical**: only the configured currency pairs are
  requested (not a whole table), and past rates can be retrieved by timestamp
  or date range (daily/hourly/minute granularity), useful for backfill and
  corrections.
