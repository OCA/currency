...
class TestResCurrencyRateProviderOXR(common.TransactionCase):
    def setUp(self):
        ...
        self.oxr_provider = self.CurrencyRateProvider.create(
            {"service": "OXR", "currency_ids": [(4, self.eur_currency.id)]}
        )
    ...
    "EUR": "Euro"
        ...
        with mock.patch(
            _provider_class + "._oxr_provider_retrieve", return_value=mocked_response,
        ):
            supported_currencies = self.oxr_provider._get_supported_currencies()
