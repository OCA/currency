...
class ResCurrencyRateProviderOXR(models.Model):
    ...
    service = fields.Selection(selection_add=[("OXR", "OpenExchangeRates.org")])
    ...
    def _get_supported_currencies(self):
        ...
        if "error" in data and data["error"]:
            raise UserError(
                data["description"] if "description" in data else "Unknown error"
            )
    ...
    def _obtain_rates(self, base_currency, currencies, date_from, date_to):
        ...
        while date <= date_to:
            url = (
                "https://openexchangerates.org/api/historical" + "/%(date)s.json"
                "?base=%(from)s" + "&symbols=%(to)s"
            ) % {"from": base_currency, "to": ",".join(currencies), "date": str(date)}
            data = json.loads(self._oxr_provider_retrieve(url))
            if "error" in data and data["error"]:
                raise UserError(
                    data["description"] if "description" in data else "Unknown error"
    ...
    def _oxr_provider_urlopen(self, url):
        ...
        if not self.company_id.openexchangerates_app_id:
            raise UserError(_("No OpenExchangeRates.org credentials specified!"))
        ...
        parsed_url = parsed_url._replace(
            query=urllib.parse.urlencode(
                parsed_query, doseq=True, quote_via=urllib.parse.quote
            )
        )
        ...
        request.add_header(
            "Authorization", "Token %s" % self.company_id.openexchangerates_app_id
        )
